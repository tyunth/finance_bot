...
// УЛУЧШЕНО: Экранирование для MarkdownV2 / Markdown
function escapeMarkdown(text) {
    if (!text) return '';
    // Экранируем символы, которые ломают разметку Telegram Markdown
...
// НОВАЯ КОМАНДА: Показать сырой текст OCR
...
bot.on('photo', async (ctx) => {
    try {
        ...
        // ВСЕГДА сохраняем сырой текст для отладки
        ctx.session.receipt = {
...
async function finalizeReceipt(ctx) {
    ...
    // Кнопка для дебага даже после успеха (можно убрать, если мешает)
...
bot.launch().then(() => {
    ...
    console.log('helloworld'); // <-- Добавлено по твоему запросу
});
